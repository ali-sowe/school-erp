...
import { useCreateInvoice, useBulkGenerateInvoices } from '@/hooks/finance/useInvoiceMutations';
...
function FinancePage() {
  ...
  const bulkGenerateInvoices = useBulkGenerateInvoices();
  ...
  const handleBulkGenerate = async (event) => {
    event.preventDefault();
    if (!bulkClassId || !bulkFeeStructureId) return;

    try {
      const created = await bulkGenerateInvoices.mutateAsync({
        feeStructureId: Number(bulkFeeStructureId),
        classId: Number(bulkClassId),
      });
      toast.success(t('invoices.bulkForm.toasts.created', { count: created?.length ?? 0 }));
      setBulkDialogOpen(false);
      setBulkClassId('');
      setBulkFeeStructureId('');
    } catch (error) {
      toast.error(error?.response?.data?.message || t('invoices.bulkForm.toasts.error'));
    }
  };
  ...
  return (
    <div className="space-y-space-6">
      ...
      <Dialog open={bulkDialogOpen} onOpenChange={setBulkDialogOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{t('invoices.bulkForm.title')}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleBulkGenerate} className="space-y-space-4">
            <p className="text-sm text-muted-foreground">{t('invoices.bulkForm.description')}</p>
            <div className="space-y-space-2">
              <label className="text-sm font-medium">{t('invoices.bulkForm.classLabel')}</label>
              <ClassSelector value={bulkClassId} onChange={setBulkClassId} params={{ status: 'ACTIVE' }} />
            </div>
            <div className="space-y-space-2">
              <label className="text-sm font-medium">{t('invoices.form.feeStructureLabel')}</label>
              <Select value={bulkFeeStructureId} onValueChange={setBulkFeeStructureId}>
                <SelectTrigger>
                  <SelectValue placeholder={t('invoices.form.feeStructurePlaceholder')} />
                </SelectTrigger>
                <SelectContent>
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={bulkGenerateInvoices.isPending}>
                {bulkGenerateInvoices.isPending
                  ? t('common:states.loading', { ns: 'common' })
                  : t('invoices.bulkForm.submitButton')}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
